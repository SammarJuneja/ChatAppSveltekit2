<script>
    ...
    let typingUser = null;
    let typingTimeout;
    let messOptions = false;
    let username =  data.user.userGet.username;
    ...
    function handleInput() {
        ...
        clearTimeout(typingTimeout);
        typingTimeout = setTimeout(clearTypingNotification, 1000);
    }
...
<div class="bg-app-bg min-h-screen">
    <div>
        <!-- header -->
        <header class="bg-black gap-2 w-full fixed flex items-center text-white text-lg p-3">
        ...
        <!-- main chat -->
        <main class="flex-1 pt-20 pb-36 overflow-y-auto">
                ...
                <div class={chat.sender === data.author.userGet._id ? "grid w-full justify-end text-right break-all pr-5" : "grid w-full justify-start text-left break-all pl-5"}>
                    <span>
                        <div class="my-2 ounded-md">
                            <button on:click={messageOptions} class="bg-signup-button px-2 py-1 w-full max-w-52 rounded-lg break-all text-white">{chat.message}</button>
                            {#if messOptions}
                                <div class="bg-black text-white p-2">Delete</div>
                            {/if}
                        </div>
                    </span>
                </div>
        ...
        <!--  bottom bar -->
        <nav class="bottom-0 fixed overflow-auto border-t w-full bg-app-bg">
